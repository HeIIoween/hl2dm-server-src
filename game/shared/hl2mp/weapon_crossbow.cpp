...
bool CCrossbowBolt::CreateSprites( void )
{
	...
	m_pGlowSprite = CSprite::SpriteCreate( "sprites/glow01.vmt", GetLocalOrigin(), false );

	if ( m_pGlowSprite != NULL )
	{
		m_pGlowSprite->FollowEntity( this );
		m_pGlowSprite->SetTransparency( kRenderTransAdd, 255, 255, 255, 255, kRenderFxNone );
		m_pGlowSprite->SetScale( 0.1f );
	...
	if ( m_pGlowTrail != NULL )
	{
		...
		m_pGlowTrail->SetTransparency( kRenderTransAdd, 255, 255, 255, 255, kRenderFxNone );
	...
	else
	{
		...
		if ( pOther->GetMoveType() == MOVETYPE_NONE && !( tr.surface.flags & SURF_SKY ) )
		{
			...
			{	
				...
				if ( m_pGlowSprite != NULL )
				{
					m_pGlowSprite->SetTransparency( kRenderTransAdd, 0, 255, 0, 255, kRenderFxNone );
...
{
	DispatchParticleEffect( "hunter_projectile_explosion_1", GetAbsOrigin(), GetAbsAngles(), NULL );
	EmitSound("NPC_Hunter.FlechetteExplode");
	RadiusDamage( CTakeDamageInfo( this, GetOwnerEntity(), 25.0, DMG_DISSOLVE ), GetAbsOrigin(), 150.0, CLASS_NONE, NULL );
	UTIL_ScreenShake( GetAbsOrigin(), 25.0, 150.0, 1.0, 750.0, SHAKE_START );
	UTIL_Remove( this );
}
...
void CWeaponCrossbow::ToggleZoom( void )
{
	...
	else
	{
		...
		{
			//engine->ClientCommand( pPlayer->edict(), "r_screenoverlay hl2mp.ru/scope02.vmt" );
			m_bInZoom = true;
		}
	}
#endif
