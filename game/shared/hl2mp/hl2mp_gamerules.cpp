...
CBaseCombatWeapon* IsManagedObjectAWeapon2( CBaseEntity *pObject )
{
	return dynamic_cast< CBaseCombatWeapon* >( pObject );
}
...
bool GetObjectsOriginalParameters( CBaseEntity *pObject, Vector &vOriginalOrigin, QAngle &vOriginalAngles )
{
	...
	else if ( CBaseCombatWeapon *pWeapon = IsManagedObjectAWeapon2( pObject ) ) 
	{
		if ( pWeapon->m_flNextResetCheckTime > gpGlobals->curtime )
			 return false;

		vOriginalOrigin = pWeapon->m_vOriginalSpawnOrigin;
		vOriginalAngles = pWeapon->m_vOriginalSpawnAngles;
...
bool CHL2MPRules::CanHavePlayerItem( CBasePlayer *pPlayer, CBaseCombatWeapon *pItem )
{
	if ( weaponstay.GetInt() > 0 )
	{
		if ( pPlayer->Weapon_OwnsThisType( pItem->GetClassname(), pItem->GetSubType() ) && !FClassnameIs( pItem, "weapon_frag") && !FClassnameIs( pItem, "weapon_slam") ) {
			return false;
		}
	}
...
{
	CBasePlayer *pOwner = (CBasePlayer *)CBaseEntity::Instance(pClient);
	if (pOwner) {
		CBaseEntity *ent = NULL;
		while ((ent = gEntList.NextEnt(ent)) != NULL)
		{
			if (ent->GetPlayerMP() && ent->GetPlayerMP() == pOwner) {
				ent->SetPlayerMP( NULL );
				UTIL_Remove(ent);
			}
		}
	}
...
ConVar mp_noblock("mp_noblock", "1");
...
bool CHL2MPRules::ShouldCollide(int collisionGroup0, int collisionGroup1)
{
	...
	if (collisionGroup0 == COLLISION_GROUP_VEHICLE && collisionGroup1 == COLLISION_GROUP_VEHICLE) {
		return false;
	}

	if (collisionGroup0 == COLLISION_GROUP_PLAYER && collisionGroup1 == COLLISION_GROUP_VEHICLE) {
		return false;
	}

	if (collisionGroup0 == COLLISION_GROUP_VEHICLE && collisionGroup1 == COLLISION_GROUP_PLAYER) {
		return false;
	}
