...
ConVar sk_max_flechette_round("sk_max_flechette_round","120");
ConVar sk_flechette_interval_shoot("sk_flechette_interval_shoot","0.2");

void CWeaponPistol::SecondaryAttack( void )
{
	float interval = sk_flechette_interval_shoot.GetFloat();
	...
	if( pOwner->GetAmmoCount( m_iSecondaryAmmoType ) <= 0 ) {
		...
		m_flNextSecondaryAttack = gpGlobals->curtime + interval;
		return;
	}
	...
	engine->ClientCommand( pOwner->edict(), "hunter_shoot_flechette_wR6yc25uhl2mpru" ); 
	SendWeaponAnim( ACT_VM_PRIMARYATTACK );
	pOwner->SetAnimation( PLAYER_ATTACK1 );
	pOwner->EmitSound("NPC_Hunter.FlechetteShoot");
	pOwner->RemoveAmmo( 1, m_iSecondaryAmmoType );
	m_flNextPrimaryAttack = gpGlobals->curtime + 0.5f;
	pOwner->ViewPunch( QAngle( -1, random->RandomFloat( -1, 1 ), 0 ) );
	m_flNextSecondaryAttack = gpGlobals->curtime + interval;
}
