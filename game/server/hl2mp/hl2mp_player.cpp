...
/*	if ( ( m_nButtons & ( IN_FORWARD | IN_BACK | IN_MOVELEFT | IN_MOVERIGHT ) ) )
	...
	else
	{
		...
		{
			bool set = true;
			if( idealActivity == ACT_HL2MP_IDLE || idealActivity == ACT_HL2MP_IDLE_CROUCH ) {
				idealActivity = ACT_IDLE;
			}
			
			if( idealActivity == ACT_HL2MP_WALK_CROUCH ) {
				idealActivity = ACT_WALK;
				if( SelectWeightedSequence( idealActivity ) == -1 ) {
					animDesired = SelectWeightedSequence( ACT_RUN );
					set = false;
				}
			}
			
			if( idealActivity == ACT_HL2MP_RUN ) {
			...
			if( idealActivity == ACT_HL2MP_JUMP ) {
