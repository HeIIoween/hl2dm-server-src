...
ConVar	sk_hgrunt_health( "sk_hgrunt_health","200");
ConVar  sk_hgrunt_kick ( "sk_hgrunt_kick", "50" );
ConVar  sk_hgrunt_pellets ( "sk_hgrunt_pellets", "10" );
...
void CNPC_HGrunt::Spawn()
{
	...
	SetModel( "models/hl2/hgrunt.mdl" );
...
void CNPC_HGrunt::Precache()
{
	...
	PrecacheModel("models/hl2/hgrunt.mdl");
		...
		while ( ( pEntity = gEntList.FindEntityInSphere( pEntity, GetAbsOrigin(), searchRadius ) ) != NULL )
		{
			...
			if ( pRecruit && pRecruit != this && pRecruit->IsAlive() && !pRecruit->m_hCine )
			{
				...
				if ( !pRecruit->m_pSquad && pRecruit->Classify() == iMyClass &&
				   ( (iMyClass != CLASS_COMBINE) || FClassnameIs( this, pRecruit->GetClassname() ) ) &&
...
void CNPC_HGrunt::Shoot ( void )
{
	...
	FireBullets(1, vecShootOrigin, vecShootDir, VECTOR_CONE_3DEGREES, 2048, m_iAmmoType ); // shoot +-5 degrees
