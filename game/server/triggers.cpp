...
{
	CBaseEntity *pOwner = pOther->GetOwnerEntity();
	if( (pOwner && FClassnameIs(pOwner,"info_npc_random")) ||
		FClassnameIs(pOther,"npc_merchant") )
		return false;

	if( pOther->IsNPC() && pOther->GetPlayerMP() )
		pOther = pOther->GetPlayerMP();
...
{
	if( inputdata.pCaller && FClassnameIs( inputdata.pCaller,"game_ui" ) ) {
...
void CTriggerCamera::Enable( void )
{
	...
	while ((pCamera = gEntList.FindEntityByClassname(pCamera, "point_viewcontrol")) != NULL)
	{
		CTriggerCamera *pOtherCamera = dynamic_cast<CTriggerCamera *>(pCamera);
		if( pOtherCamera && pOtherCamera != this ) {
	...
	if ( !m_hPlayer )
	{
		for (int i = 1; i <= gpGlobals->maxClients; i++)
		{
			...
			if ( pPlayer && pPlayer->IsConnected() && pPlayer->GetViewEntity() == pPlayer ) {
	...
	{
		m_pPath = gEntList.FindEntityByName( NULL, m_sPath, NULL );
	}
	else
	{
		m_pPath = NULL;
...
void CTriggerCamera::Disable( void )
{
	...
	{
		CBasePlayer *pPlayer = UTIL_PlayerByIndex(i);
		if ( pPlayer && pPlayer->IsConnected() && pPlayer->GetViewEntity() == this ) {
			if( HasSpawnFlags( SF_CAMERA_PLAYER_NOT_SOLID ) )
