...
void CAI_LeadBehavior::LeadPlayer( const AI_LeadArgs_t &leadArgs, CAI_LeadBehaviorHandler *pSink )
{
	...
	if( pOuter ) 
	{
		CBasePlayer *pPlayer = UTIL_GetNearestPlayer( pOuter->GetAbsOrigin() );
		if( pPlayer )
...
void CAI_LeadBehavior::GatherConditions( void )
{
	...
	if ( HasGoal() )
	{
		...
		CBaseEntity *pFollower = NULL;
		CAI_BaseNPC *pNpc = GetOuter();
		if( pNpc )
			pFollower = UTIL_GetNearestPlayer( pNpc->GetAbsOrigin() );
	...
	switch ( pTask->iTask )
	{
		...
		case TASK_LEAD_RETRIEVE_WAIT:
		{
			...
			if( pNpc )
				pFollower = UTIL_GetNearestPlayer( pNpc->GetAbsOrigin() );
			if( pFollower )
				m_MoveMonitor.SetMark( pFollower, 24 );
