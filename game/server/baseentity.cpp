...
{
	if( GetPlayerMP() && info.GetAttacker() && info.GetAttacker()->IsPlayer() )
		return 0;
...
#endif
	DEFINE_KEYFIELD( m_iszCustomID, FIELD_STRING, "CustomID" ),
	DEFINE_KEYFIELD( m_iszCustomID, FIELD_STRING, "npcname" ),
END_DATADESC()
...
{
	CBaseEntity *pOwner = GetPlayerMP();
	if ( pOwner )
	{
...
void CBaseEntity::SetLocalAngles( const QAngle& newangles )
{
