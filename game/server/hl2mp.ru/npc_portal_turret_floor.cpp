...
ConVar portal_turret_rgb("portal_turret_rgb","255 165 0");
...
#define	FLOOR_TURRET_PORTAL_MODEL_DEFECTIVE	"models/npcs/turret/turret_skeleton.mdl"
...
#define	PORTAL_FLOOR_TURRET_RANGE	16000
...
const char *g_TalkNamesDefective[] = 
{
	"NPC_FloorTurretDefective.TalkSearch",
	"NPC_FloorTurretDefective.TalkAutosearch",
	"NPC_FloorTurretDefective.TalkActive",
	"NPC_FloorTurretDefective.TalkSupress",
	"NPC_FloorTurretDefective.TalkDeploy",
	"NPC_FloorTurretDefective.TalkRetire",
	"NPC_FloorTurretDefective.TalkTipped",
	NULL							// Must have NULL at end of list in case more states are added to base turret!
};

const char *g_PortalTalkNamesDefective[ PORTAL_TURRET_STATE_TOTAL - TURRET_STATE_TOTAL ] = 
{
	"NPC_FloorTurretDefective.TalkDisabled",
	"NPC_FloorTurretDefective.TalkCollide",
	"NPC_FloorTurretDefective.TalkPickup",
	"NPC_FloorTurretDefective.TalkShotAt",
	"NPC_FloorTurretDefective.TalkDissolved"
};
...
public:
	...
	virtual const char* GetTurretTalkName( int iState );
...
LINK_ENTITY_TO_CLASS( npc_portal_turret_rpg, CNPC_Portal_FloorTurret );
LINK_ENTITY_TO_CLASS( npc_portal_turret_defective, CNPC_Portal_FloorTurret );
//Datatable
...
const char* CNPC_Portal_FloorTurret::GetTurretTalkName( int iState )
{
	if( FClassnameIs( this, "npc_portal_turret_defective") ) {
		if ( iState < TURRET_STATE_TOTAL )
			return g_TalkNamesDefective[ iState ];
	
		return g_PortalTalkNamesDefective[ iState - TURRET_STATE_TOTAL ];
	}
	else {
		if ( iState < TURRET_STATE_TOTAL )
...
{
	if( FClassnameIs( this, "npc_portal_turret_defective") ) {
		SetModelName( MAKE_STRING( FLOOR_TURRET_PORTAL_MODEL_DEFECTIVE ) );
	}
	else {
	...
	m_sLaserHaloSprite = PrecacheModel( "sprites/glow1.vmt" );
	...
	for ( int iTalkScript = 0; iTalkScript < PORTAL_TURRET_STATE_TOTAL; ++iTalkScript )
	{
		...
		{
			if ( g_TalkNamesDefective[ iTalkScript ] )
				PrecacheScriptSound( g_TalkNamesDefective[ iTalkScript ] );
			else
				iTalkScript = TURRET_STATE_TOTAL - 1;
		...
		else
		{
			...
			PrecacheScriptSound( g_PortalTalkNamesDefective[ iTalkScript - TURRET_STATE_TOTAL ] );
		}
	}
}
...
{ 
	...
	else if( FClassnameIs( this, "npc_portal_turret_rpg") ) {
		m_nSkin = 1;
	}
	else {
		m_nSkin = 0;
...
{
	if ( m_lifeState == LIFE_ALIVE && ( info.GetDamageType() & DMG_BULLET ) )
	{
...
{
	if( FClassnameIs( this, "npc_portal_turret_rpg") ) {
		if( !m_hMissile && m_fNextRgb < gpGlobals->curtime) {
			QAngle vecAngles;
			VectorAngles( vecDirToEnemy, vecAngles );
			CMissile *pMissile = CMissile::Create( vecSrc, vecAngles, edict() );
			if( pMissile ) {
				pMissile->SetDamage( 50.0 );
				m_hMissile = pMissile;
			}
			EmitSound( "Weapon_IRifle.Single" );
			m_fNextRgb = gpGlobals->curtime + RandomFloat(1.0, 2.0);
		}
	}
...
void CNPC_Portal_FloorTurret::LaserOn( void )
{
	...
	if(m_pBeam) {
		Vector vEndPoint = vecMuzzle + vecMuzzleDir * FLOOR_TURRET_PORTAL_LASER_RANGE;	// Trace hit a wall
		trace_t tr;
		UTIL_TraceLine(vecMuzzle,vEndPoint,MASK_SHOT,this,COLLISION_GROUP_PLAYER,&tr);
		if( tr.fraction != 1.0 ) {
			m_pBeam->PointsInit( tr.endpos, vecMuzzle );
		}
		else {
			m_pBeam->PointsInit( vEndPoint, vecMuzzle );
		}
		const char *rgb = portal_turret_rgb.GetString();
		if(GetEnemy() == NULL)
			m_pBeam->KeyValue( "rendercolor", rgb );
		else
			m_pBeam->KeyValue( "rendercolor", "255 0 0" );
...
float CNPC_Portal_FloorTurret::LaserEndPointSize( void )
{
