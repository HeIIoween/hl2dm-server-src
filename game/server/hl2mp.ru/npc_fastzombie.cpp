...
ConVar sk_fastzombie_health("sk_fastzombie_health","100");
ConVar sk_fastzombie_dmg_leap("sk_fastzombie_dmg_leap","40");
ConVar sk_fastzombie_dmg_claw("sk_fastzombie_dmg_claw","14");
...
LINK_ENTITY_TO_CLASS( npc_fast, CFastZombie );
...
void CFastZombie::Precache( void )
{
	...
	PrecacheModel("models/hl2/fast_torso.mdl");
#ifdef HL2_EPISODIC
...
void CFastZombie::Spawn( void )
{
	...
	m_iHealth = m_iMaxHealth = sk_fastzombie_health.GetInt();
	m_flFieldOfView		= 0.2;
...
void CFastZombie::SetZombieModel( void )
{
	...
	if ( m_fIsTorso )
	{
		SetModel( "models/hl2/fast_torso.mdl" );
...
void CFastZombie::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_ZOMBIE_ATTACK_RIGHT )
	{
		...
		QAngle angle( -3, -5, -3  );
		ClawAttack( GetClawAttackRange(), sk_fastzombie_dmg_claw.GetInt(), angle, right, ZOMBIE_BLOOD_RIGHT_HAND );
		return;
	}
	...
	if ( pEvent->event == AE_ZOMBIE_ATTACK_LEFT )
	{
		...
		right = right * 50;
		QAngle angle( -3, 5, -3 );
		ClawAttack( GetClawAttackRange(), sk_fastzombie_dmg_claw.GetInt(), angle, right, ZOMBIE_BLOOD_LEFT_HAND );
		return;
	}
...
void CFastZombie::LeapAttackTouch( CBaseEntity *pOther )
{
	...
	ClawAttack( GetClawAttackRange(), sk_fastzombie_dmg_leap.GetInt(), qaPunch, forward, ZOMBIE_BLOOD_BOTH_HANDS );
