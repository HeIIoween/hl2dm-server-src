...
#define ROCKET_TURRET_LOCKON_TIME 1.0f
...
{ 
	SetThink( &CNPC_RocketTurret::LaserThink );
...
void CNPC_RocketTurret::LaserThink( void )
{
	SetNextThink( gpGlobals->curtime + 0.05 );
	//if ( m_iLaserState != 0 )
		LaserOn();
...
void CNPC_RocketTurret::Activate( void )
{
	...
	m_filterBeams.SetPassEntity2( UTIL_GetNearestPlayer( GetAbsOrigin() ) );
	BaseClass::Activate();
}
...
{
	if( UTIL_GetNearestPlayer( GetAbsOrigin() ) )
		SetEnemy( UTIL_GetNearestPlayer( GetAbsOrigin() ) );
...
bool CNPC_RocketTurret::TestLOS( const Vector& vAimPoint )
{
	...
	//UTIL_TraceRay( ray, MASK_VISIBLE_AND_NPCS, &filter, &trTarget, false );
...
void CNPC_RocketTurret::LaserOn( void )
{
	...
	Vector vEndPoint = vecMuzzle + vecAimDir * ROCKET_TURRET_LASER_RANGE;	// Trace hit a wall
	trace_t tr;
	...
	m_pBeam->SetHaloScale( ( max( 0.0f, sinf( gpGlobals->curtime * M_PI + RandomFloat( 0.0f, 2.0f * M_PI ) ) ) ) * ROCKET_TURRET_END_POINT_PULSE_SCALE + 1.0f );
	m_iLaserState = 1;*/
}
