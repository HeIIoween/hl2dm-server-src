...
ConVar sk_plr_dmg_gauss( "sk_plr_dmg_gauss", "35" );
ConVar sk_plr_max_dmg_gauss( "sk_plr_max_dmg_gauss", "150" );
...
void CWeaponGaussGun::ChargedFire( void )
{
	...
	if ( penetrated == true )
	{
		RadiusDamage( CTakeDamageInfo( this, this, flDamage, DMG_SHOCK ), tr.endpos, 200.0f, CLASS_PLAYER, NULL );
	}
...
ConVar sk_weapon_gauss_spin_speed("sk_weapon_gauss_spin_speed","10");
...
void CWeaponGaussGun::PrimaryAttack( void )
{
	...
	m_nSpinPos -= sk_weapon_gauss_spin_speed.GetInt();
	vm->SetPoseParameter( "coil_spin", m_nSpinPos );

	if( m_flNextFire >= gpGlobals->curtime - 0.1 ) {
	...
	m_flNextFire = gpGlobals->curtime + GetFireRate();
...
// Purpose: 
//-----------------------------------------------------------------------------
ConVar sk_weapon_gauss_spincharge_speed("sk_weapon_gauss_spincharge_speed","10");
void CWeaponGaussGun::IncreaseCharge( void )
{
	...
	m_nSpinPos -= sk_weapon_gauss_spincharge_speed.GetInt();
	vm->SetPoseParameter( "coil_spin", m_nSpinPos );
