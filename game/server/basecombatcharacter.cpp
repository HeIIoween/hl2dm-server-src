...
// UNDONE: Put in a real sound cue here, don't do this bogus hack anymore
	...
	if ( m_bForceServerRagdoll == true || ( ( bMegaPhyscannonActive == true ) && !IsPlayer() && Classify() != CLASS_PLAYER_ALLY_VITAL && Classify() != CLASS_PLAYER_ALLY ) )
	{
		...
		CBaseEntity *pRagdoll = CreateServerRagdoll( this, m_nForceBone, newinfo, COLLISION_GROUP_DEBRIS, true );
		if( pRagdoll ) {
			FixupBurningServerRagdoll( pRagdoll );
...
void CBaseCombatCharacter::Weapon_Drop( CBaseCombatWeapon *pWeapon, const Vector *pvecTarget /* = NULL */, const Vector *pVelocity /* = NULL */ )
{
	...
	if ( HasSpawnFlags( SF_NPC_NO_WEAPON_DROP ) || pWeapon->HasSpawnFlags( SF_NPC_NO_WEAPON_DROP ) )
	{
...
int CBaseCombatCharacter::OnTakeDamage_Alive( const CTakeDamageInfo &currentinfo )
{
		...
		if( pPlayer && IsNPC() && !FClassnameIs( this, "npc_barnacle" ) && !FClassnameIs( this, "npc_furniture" ) ) {
			...
			if( flIntegerDamage < m_iHealth ) {
				if( dmgpnpc )
					...
					m_hTestPL[pPlayer->entindex()] += flIntegerDamage;
			}
			else
...
Disposition_t CBaseCombatCharacter::IRelationType ( CBaseEntity *pTarget )
{
	if( pTarget ) {
		...
		if( pTarget->pfriend || pTarget->GetPlayerMP() || ( FClassnameIs( pTarget, "npc_merchant" ) && pTarget->Classify() != CLASS_NONE ) ) {
			if( IsPlayer() )
				return D_LI;
		...
		if( pfriend || GetPlayerMP() || ( FClassnameIs( this, "npc_merchant" ) && Classify() != CLASS_NONE  ) ) {
			...
			if( tOwner && FClassnameIs( tOwner, "info_npc_random" ) )
				return D_HT;

			CBasePlayer *pPlayer = UTIL_GetLocalPlayer();
			CAI_BaseNPC *pNPC = dynamic_cast< CAI_BaseNPC * >(pTarget);
			if( pPlayer && pNPC )
				return pNPC->FindEntityRelationship(pPlayer)->disposition;
		}

		CBaseEntity *myOwner = GetOwnerEntity();
		if( myOwner && FClassnameIs( myOwner, "info_npc_random" ) && pTarget->IsPlayer() )
			return D_HT;

		CBaseEntity *tOwner = pTarget->GetOwnerEntity();
		if( tOwner && FClassnameIs( tOwner, "info_npc_random" ) && IsPlayer() )
			return D_HT;
